...
**ojo-agent** 是一个强大的OJ题目批处理平台，通过AI大语言模型自动化完成题目的全生命周期管理。无论你是需要批量迁移题目、自动生成测试数据，还是构建自己的OJ平台，ojo-agent都能帮你显著提升效率。
...
从多个OJ平台智能拉取题目，支持：
- **已支持平台**：
  - ✅ **SHSOJ** - 完整支持，支持按标签批量拉取
  - ✅ **Aicoders** - 完整支持
  - ✅ **HydroOJ** - 完整支持
  - ✅ **Manual** - 手动题面输入
  - ⚠️ **Codeforces** - 仅支持题面拉取和官方题解（待开发：数据上传）
  - ⚠️ **AtCoder** - 仅支持题面拉取（待开发：数据上传、题解）
  - ⚠️ **洛谷(Luogu)** - 仅支持题面拉取和官方题解（待开发：数据上传）
...
AI自动生成题解并验证：
- **已支持平台**：
  - ✅ **SHSOJ** - 完整支持代码自动提交和判题结果追踪
  - ✅ **HydroOJ** - 完整支持代码自动提交和判题结果追踪
  - ⏳ **Codeforces** - 待开发代码提交功能（仅支持官方题解获取）
  - ⏳ **AtCoder** - 待开发
  - ⏳ **洛谷(Luogu)** - 待开发代码提交功能（仅支持官方题解获取）
...
| OJ平台 | 题面拉取 | 数据上传 | 代码提交 | 题单管理 | 官方题解 | 状态 |
|--------|---------|---------|---------|---------|---------|------|
| **SHSOJ** | ✅ | ✅ | ✅ | ✅ | ❌ | 完整支持 |
| **HydroOJ** | ✅ | ✅ | ✅ | ❌ | ❌ | 完整支持 |
| **Aicoders** | ✅ | ❌ | ❌ | ❌ | ❌ | 仅拉取 |
...
> 💡 **提示**：完整功能需要配置对应平台的账号信息。公开平台（Codeforces、AtCoder、Luogu）目前仅支持题面拉取，数据上传和代码提交功能正在开发中。
...
**需要配置的平台**：
- **SHSOJ**：需要用户名和密码（完整功能需配置）
- **HydroOJ**：需要base_url、domain和Cookie/Token（完整功能需配置）

**无需配置的平台**（公开API）：
- **Codeforces**：无需配置（但仅支持题面拉取和官方题解）
- **AtCoder**：无需配置（但仅支持题面拉取）
- **洛谷(Luogu)**：无需配置（但仅支持题面拉取和官方题解）
- **Aicoders**：无需配置（仅支持题面拉取）

> 💡 **提示**：如果需要使用数据上传和代码提交功能，请确保配置了对应平台的账号信息。
...
1002
2000-2005
https://oj.example.com/problem/1234
```
...
ojo-agent提供完整的REST API，支持程序化集成。
...
```bash
# 构建镜像
docker build -t ojo-agent:latest .
...
```ini
[Unit]
Description=ojo-agent API Server
...
```bash
sudo systemctl enable ojo-agent
sudo systemctl start ojo-agent
sudo systemctl status ojo-agent
```
...
A: 目前完整支持的平台有：
- **SHSOJ** - 完整功能（拉取、上传、提交、题单管理）
- **HydroOJ** - 完整功能（拉取、上传、提交）

部分支持的平台：
- **Codeforces** - 支持拉取题面和获取官方题解，数据上传和代码提交功能待开发
- **AtCoder** - 支持拉取题面，其他功能待开发
- **洛谷(Luogu)** - 支持拉取题面和获取官方题解，数据上传和代码提交功能待开发

基础支持的平台：
- **Aicoders** - 仅支持题面拉取
- **Manual** - 手动题面输入

更多平台功能正在开发中，详情请查看 [OJ平台支持情况](#-oj平台支持情况)。
